/********************************************************************
	Created:	2016-04-15  11:31:41
	Filename: 	AVRGameConsole2.3\PlatformDrv\LCD\font8x14.cpp

	Author:		Unknown

	purpose:	Monowidth font 8x14 (with cyrillic symbols win-1251)
*********************************************************************/

//------------------------------------- Настройка шрифта ----------------------------------------------------

#define FONT8X14_USE_8					//Использовать функцию для печати 8-ми битным цветом
//#define FONT8X14_USE_16				//Использовать функцию для печати 16-ти битным цветом
#define FONT8X14_USE_8BGL				//Использовать функцию для печати 8-ми битным цветом без фона
//#define FONT8X14_USE_16BGL			//Использовать функцию для печати 16-ти битным цветом без фона
...
static const unsigned char table8x14[] PROGMEM = {
...
void put_char8x14_8(const Coord_t x, const Coord_t y, const char c, const color8 color, const color8 bgcolor) {
	unsigned char mask, cl, ch;
	auto base = &table8x14[14 * (static_cast<unsigned char>(c) - 0x20)];		//cast to unsigned char to work with cp1251
	lcd::SetWindow(x, y, 8, 14);
	lcd::SetDatMode();
	for (cl = 0; cl < 14; cl++) {
...
void put_char8x14_16(const Coord_t x, const Coord_t y, const char c, const color16 color, const color16 bgcolor) {
	unsigned char mask, cl, ch;
	auto base = &table8x14[14 * (static_cast<unsigned char>(c) - 0x20)];
	lcd::SetWindow(x, y, 8, 14);
	lcd::SetDatMode();
	for (cl = 0; cl < 14; cl++) {
		ch = pgm_read_byte(&base[cl]);
...
void put_char8x14_8BGL(const Coord_t x, const Coord_t y, const char c, const color8 color) {
	unsigned char mask, cl, ch, off_x = 0, off_y = 0;
	auto base = &table8x14[14 * (static_cast<unsigned char>(c) - 0x20)];
...
void put_char8x14_16BGL(const Coord_t x, const Coord_t y, const char c, const color16 color) {
	unsigned char mask, cl, ch, off_x = 0, off_y = 0;
	auto base = &table8x14[14 * (static_cast<unsigned char>(c) - 0x20)];
	for (cl = 0; cl < 14; cl++, off_y++) {
		ch = pgm_read_byte(&base[cl]);
...
namespace fnt {
	const font font8x14 = {
		...
		#ifdef FONT8X14_USE_8
		put_char8x14_8,
		#else
		nullptr,
		#endif //FONT8X14_USE_8

		#ifdef FONT8X14_USE_8BGL
		put_char8x14_8BGL,
		#else
		nullptr,
		#endif	//FONT8X14_USE_8BGL

		#ifdef FONT8X14_USE_16
		put_char8x14_16,
		#else
		nullptr,
		#endif //FONT8X14_USE_16

		#ifdef FONT8X14_USE_16BGL
		put_char8x14_16BGL
		#else
		nullptr
		#endif	//FONT8X14_USE_16BGL
	};
}
